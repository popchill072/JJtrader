...
async function handleLoginSubmit() {
  ...
  if (!pin || pin.length < 4) {
    if (errEl) { errEl.textContent = '❌ กรุณากรอก PIN อย่างน้อย 4 หลัก (เช่น 1234)'; errEl.classList.add('show'); }
    return;
  }
...
async function loadForexNews() {
  ...
  try {
    const res = await fetch('https://jj-trader-api.popchill072.workers.dev/api/news');
    if (res.ok) {
      currentNewsData = await res.json();
    }

    if (!Array.isArray(currentNewsData) || !currentNewsData.length) {
      container.innerHTML = '<div style="color:var(--text-muted);font-size:11px;text-align:center;padding:15px;">ไม่มีข่าวสารสำคัญในขณะนี้...</div>';
      return;
    }
...
function renderNewsListWithCountdown() {
  ...
  currentNewsData.slice(0, 15).forEach((item, index) => {
    ...
    el.innerHTML = `
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;color:var(--text-muted);font-size:10px;">
        <span>⏰ เวลา: <strong>${timeStr} น.</strong></span>
        <span id="news-cd-${index}" style="font-weight:700;color:var(--accent-cyan);">⏳ กำลังคำนวณเวลา...</span>
      </div>
